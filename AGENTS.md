...
- Дети: `src/services/child.service.ts` + `UserRepository.findChildren/findChild/unlinkChild`; `GET /children`, `GET /children/:id`, `DELETE /children/:id` (отвязать, `parent_id = null`) — только для родителя.
...
- Вайтборд: `page.service.ts` + `room-page.repository.ts`/`stroke.repository.ts` (страницы `room_pages` и штрихи `strokes`). Файлы изображений пишутся в `UPLOAD_DIR` (dev — `./uploads`, примонтирован в контейнер, в `.gitignore`). Загрузка — `@fastify/multipart`. Статика отдаётся по `/media/<file_name>` (UUID, `Cache-Control: immutable`): в проде — Caddy напрямую из `./uploads`, в dev — `@fastify/static`; старый авторизованный `GET /pages/:pageId/image` остаётся фолбэком. События WS: `page:add`, `page:remove`, `stroke:add`, `stroke:remove`. После `APPROVED` старые сообщения/миниатюры скрываются за «Архив» (cutoff — `chat_sessions.updated_at`), а непросмотренные фото подсвечиваются (id просмотренных — в `localStorage`).
...
- Уведомления: `services/notifications.ts` (`sendNotification`) шлёт второму участнику в персональный канал событие `notification` (`kind`: `message`/`status`/`image`) — из `ChatService.createMessage` (только не системные), `ChatSessionService.review` и `PageService.create`. Фронт — `composables/useNotifications.ts` (Notification API, дебаунс, только при скрытой вкладке) + `composables/useToasts.ts` (тосты в правом нижнем углу на дашборде) и счётчик в `document.title`.
...
- Изображения вайтборда грузятся напрямую с backend (`runtimeConfig.public.backendOrigin` + `/media/<file_name>`), т.к. бинарный ответ не идёт через прокси; в проде путь `/media/*` обслуживает Caddy.
