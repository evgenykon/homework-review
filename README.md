...
| `DELETE` | `/api/children/:id` | Отвязать ребёнка от родителя (`parent_id = null`) |
...
В карточке «Дети» родитель управляет списком: **«Отвязать»** у ребёнка (`DELETE /api/children/:id` — снимает связь `parent_id`, аккаунт и комнаты сохраняются) и **«+ Добавить ребёнка»** — генерирует инвайт-ссылку для входа нового ребёнка.
...
- Новые (не просмотренные пользователем) фото подсвечиваются пульсирующей рамкой; после открытия фото подсветка гаснет (просмотренные id хранятся в `localStorage`).
- Рядом с «Рисовать» — кнопка **«Калькулятор»** (модалка с клавиатурой и поддержкой клавиш).
...
- Изображения сохраняются на диск в `UPLOAD_DIR` (в dev — каталог `uploads/` в корне репозитория, примонтирован в контейнер, в `.gitignore`) и отдаются статикой по `/media/<file_name>`: в проде — напрямую Caddy из смонтированного каталога, в dev — Fastify (`@fastify/static`). Имена файлов — UUID, ответ кешируется навсегда (`Cache-Control: public, max-age=31536000, immutable`), поэтому повторные открытия идут из кеша браузера и не нагружают Node.
...
- Пока статус `APPROVED`, старые сообщения и миниатюры фото скрыты за кнопкой **«Архив»** (клик разворачивает историю); новые сообщения и фото, появившиеся после одобрения, видны сразу.
...
- Книгу можно открыть (`GET /api/books/:id/file`, отдаётся inline — браузер показывает PDF) и удалить (`DELETE /api/books/:id`, файл стирается с диска). Файлы книг отдаются с `Cache-Control: private, max-age=31536000, immutable`, поэтому повторное открытие не скачивает файл заново.
...
Помимо системного уведомления, при событии на дашборде всплывает тост в правом нижнем углу (исчезает через 5 секунд), а в `title` страницы появляется счётчик непрочитанных уведомлений (сбрасывается при возврате фокуса на вкладку).
...
- **Caddy** — единственная публичная точка (порты 80/443), автоматически получает сертификат Let's Encrypt для `DOMAIN` и проксирует: `/api/*` → backend, `/ws` → backend, остальное → frontend. Загруженные файлы (`/media/*`) Caddy отдаёт сам из примонтированного read-only каталога `./uploads`, минуя Node.
