...
- Книгу можно открыть (`GET /api/books/:id/file`, отдаётся inline — браузер показывает PDF) и удалить (`DELETE /api/books/:id`, файл стирается с диска). Файлы книг и изображений отдаются с `Cache-Control: private, max-age=31536000, immutable`, поэтому повторное открытие не скачивает файл заново.
